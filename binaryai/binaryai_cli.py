...
def cli(ctx, cfg, help, version, install, ida_plugins):
    ...
    if not os.path.isdir(ida_path) or not os.path.exists(ida_path):
        print("$IDAUSR path not found")
        ctx.exit()

    if install:
        if ida_plugins and not os.path.isdir(ida_plugins):
            print('Invalid plugin path')
            ctx.exit()
        if not ida_plugins:
            ida_plugins = os.path.join(ida_path, "plugins")
        os.makedirs(ida_plugins) if not os.path.exists(ida_plugins) else None
