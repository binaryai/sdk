...
class BinaryAIManager(object):
    ...
    def retrieve(self, ea, topk, funcset_ids):
...
def cmd_match(funcset_ids=None):
    ...
    for ea in idautils.Functions():
        try:
            targets = bai_mgr.retrieve(ea, bai_config['topk'], funcset_ids)
        except Exception:
            continue
...
def load_ida_plugin():
    ...
    if not idaapi.is_idaq():
        return False
    bai_mgr = BinaryAIManager()
    ui_mgr = UIManager("BinaryAI", bai_mgr)
    return ui_mgr.register_actions()
